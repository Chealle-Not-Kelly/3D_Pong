...
const Ping_C_V= {
    ...
    move: (PingBall) => { 
        if (PingBall.Game_Started == true){
            ...
            case "right":
                PingBall.Ping_x += PingBall.Ping_Speed
                break;
            case "left":
                PingBall.Ping_x -= PingBall.Ping_Speed
                break;
...
function draw_ping(x,y,z,diameter){

    push();
    translate(x,y,z)
    fill("yellow")
    sphere(diameter, 10, 4)
    pop();
}
...
function draw() {
    ...
    draw_ping(Ping_C_V.Ping_x, Ping_C_V.Ping_y ,Ping_C_V.Ping_z, Ping_C_V.Ping_diameter)
    Ping_C_V.move(Ping_C_V)

    console.log(Pong_C_V.getPos())
